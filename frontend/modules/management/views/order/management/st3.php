...
<?php if($orders) foreach ($orders as $order) { 
    ...
    ?>
    <div class="item-info-donhang  <?= ($order['payment_status'] == \common\components\payments\ClaPayment::PAYMENT_STATUS_SUCCESS) ? 'payment_status'  : '' ?>" id="box-b3-<?= $order['id'] ?>">
        <div class="title">
            <div class="img">
                <a href="<?= $s_url ?>">
        ...
        <div class="title bottom-item-info-donhang">
            ...
            <div class="code-donhang">
                <a class="open-popup-link open-popup-link-b3" data-id="<?= $order['id'] ?>" href="#donhang-chuanbi3">Giao hàng thành công</a>
            </div>
        </div>
    </div>
<?php } ?>
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('.open-popup-link-b3').click(function() {
            href = '<?= Url::to(['/management/order/get-detail']) ?>';
            var shop_order = $(this).attr('data-id');
            loadAjax(href , {id : shop_order, status_get: 3} , $('#box-detail3'));
        });
    });
</script>