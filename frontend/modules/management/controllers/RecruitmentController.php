...
class RecruitmentController extends CController {
    ...
    public function beforeAction($action) {
        $recruiter = \frontend\models\UserRecruiterInfo::findOne(\Yii::$app->user->getId());
        if (!$recruiter) {
            $url = \yii\helpers\Url::to(['/management/user-recruiter-info/update']);
            \common\components\ClaLid::alertMsg('Bạn phải cập nhật thông tin nhà tuyển dụng trước khi đăng tin.', $url);
            return;
        }
