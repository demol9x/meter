...
class UserBankController extends Controller
{
    ...
    {
        $model = new UserBank();
        $model = UserBank::getAllBank(Yii::$app->user->id);
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            $addtg = UserBank::find()->where(['isdefault' => 1,'user_id' => Yii::$app->user->id])->one();
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            $model->user_id = Yii::$app->user->id;
            //
            if(!$isdefault && $model->isdefault && ($addtg = UserBank::find()->where(['isdefault' => 1])->one())) {
                $addtg->isdefault =0;
                if($model->save() && $addtg->save()) {
                    return $this->redirect(['index']);
                }
    ...
    {
        $model = $this->findModel($id);
        if($addtg = UserBank::find()->where(['isdefault' => 1])->one()) {
            $addtg->isdefault =0;
            $model->isdefault =1;
            if($model->save() && $addtg->save()) {
