...
class SearchController extends Controller
{
    ...
    public function actionSearchAjax()
    {
        ...
        $latlng = isset($get['latlng']) ? __removeDF($get['latlng']) : 0;
        $data = [];
        if (isset($get['type'])) {
            unset($get['type']);
        }
        switch ($type) {
            case ClaSearch::TYPE_PRODUCT:
                if ($latlng && count(explode(',', $latlng)) == 2) {
                    $tg = explode(',', $latlng);
                    $lats =  $tg[0];
                    $lngs =  $tg[1];
    ...
    public function actionIndex()
    {
        ...
        $get_range = 0;
        //
        if (isset($data['latlng']) && $data['latlng'] && count(explode(',', $data['latlng'])) == 2) {
            $center = __removeDF($data['latlng']);
            $tg = explode(',', $center);
            $lats =  $tg[0];
            $lngs =  $tg[1];
