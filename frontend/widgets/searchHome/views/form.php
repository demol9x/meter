...
$huong_nha = isset($get['huong_nha']) && $get['huong_nha'] ? $get['huong_nha'] : null;
?>
                ...
                <select class="none" data-placeholder="Mức giá" tabindex="5" name="price_min">
                    ...
                    <?php foreach($bds['price_fillter'] as $price_fillter) { ?>
                        <option value="<?php echo $price_fillter['min'] ?>" data-max="<?php echo $price_fillter['max'] ?>" <?= $price_min == $price_fillter['min'] && $price_max == $price_fillter['max'] ? 'selected' : '' ?>>
                            <?php echo ClaSite::formatPrice($price_fillter['max'], $price_fillter['min'])?>
                        </option>
                    <?php } ?>
                </select>
            </div>
        ...
        <div class="form-group r2" style="display: none;">
            <div class="product_select_extra">
                <select data-placeholder="Hướng nhà đất" tabindex="5" name="huong_nha">
                    <option value="" selected>Hướng nhà đất</option>
                    <?php if(isset($huong_nha_select) && $huong_nha_select) foreach($huong_nha_select as $key => $hn) { ?>
                    <option value="<?php echo $key ?>" <?= $key == 0 ? $key == $huong_nha && !is_null($huong_nha) ? 'selected' : '' : $key == $huong_nha ? 'selected' : '' ?>><?php echo $hn ?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
