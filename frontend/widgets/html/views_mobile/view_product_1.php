    ...
    foreach ($products as $tg) {
        ...
        <div class="item-product-inhome relative">
            <div class="img">
                <div class="hidden-loading-content">
                    <a href="<?= $url ?>" title="<?= $product['name'] ?>">
                        <img class="lazy" data-src="<?= ClaHost::getLinkImage($product->avatar_path, $product->avatar_name, 's200_200/') ?>" alt="<?= $product->name ?>" alt="<?= $product['name'] ?>" />
                    </a>
            ...
            <p class="price marker">
                ...
                <?php if ($price > 0) {
                    if ($product->inCatSale()) {  ?>
                        <a class="btn-style-3" href="<?= $product->getLinkBuy() ?>">Mua ngay <i class="fa fa-shopping-cart"></i>
                        </a>
                    <?php } else { ?>
