                ...
                <p class="price">
                    <?php if (!$product['price_range']) { ?>
                        ...
                        if(count($price_range) > 1) {
                            $price_range = explode(',', $product['price_range']);
                            if ($price_range[0]) { //Quang check lỗi (phải thêm đoạn này vào)
                                $price_max = number_format($price_range[0], 0, ',', '.');
                            }
                            if ($price_range[count($price_range) - 1]) { //Quang check lỗi (phải thêm đoạn này vào)
                                $price_min = number_format($price_range[count($price_range) - 1], 0, ',', '.');
                            }
                            if ($price_max && $price_min) { //Quang check lỗi (phải thêm đoạn này vào)
