                    ...
                    <?php foreach ($data as $item) { ?>
                        <div class="item <?= $item['unread'] ? 'unread' : '' ?>">
                            ...
                            <div class="account-notify-content">
                                ...
                                <p>
                                    <a href="<?= $item['link'] ?>">
                                        <span style="color: #000; margin-right: 10px;"><?= $item['description'] ?></span> 
                                    </a>
                                </p>
                                <?php if ($item['unread']) { ?>
...
<script type="text/javascript">
    ...
    function deleteNotify(notify_id, _this) {
        confirmCS('<?= Yii::t('app', 'delete_sure') ?>', {notify_id});
        _this.addClass('box-waiting-confirm-js');
        yesConFirm =  function(option) {
            _this = $('.box-waiting-confirm-js').first();
            _this.removeClass('box-waiting-confirm-js');
            notify_id = option.notify_id;
            $.getJSON(
