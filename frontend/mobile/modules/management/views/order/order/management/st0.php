...
<?php if($shop_orders) foreach ($shop_orders as $shop_order) { 
    ...
    ?>
    <div class="item-info-donhang" id="box-b0-<?= $shop_order['id'] ?>">
        <div class="title">
            <div class="img">
                <a href="<?= $s_url ?>">
        ...
        <div class="title bottom-item-info-donhang">
            ...
            <div class="code-donhang">
                <a class="open-popup-link open-popup-link-b0" data-id="<?= $shop_order['id'] ?>" href="#donhang-chuanbi0"><?= Yii::t('app', 'order_detail') ?></a>
            </div>
        </div>
    </div>
<?php } ?>
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('.open-popup-link-b0').click(function() {
            href = '<?= Url::to(['/management/order/get-detail']) ?>';
            var shop_order = $(this).attr('data-id');
            loadAjax(href , {id : shop_order, status_get: 0} , $('#box-detail0'));
        });
    });
</script>