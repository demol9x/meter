...
<?php if($orders) foreach ($orders as $order) { 
    ...
    ?>
    <div class="item-info-donhang  <?= ($order['payment_status'] == \common\components\payments\ClaPayment::PAYMENT_STATUS_SUCCESS) ? 'payment_status'  : '' ?>" id="box-b3-<?= $order['id'] ?>" id="box-b3-<?= $order['id'] ?>">
        <div class="title">
            <div class="img">
                <a href="<?= $s_url ?>">
        ...
        <div class="title bottom-item-info-donhang">
            ...
            <div class="code-donhang">
                ...
                <a class="click" onclick="received(<?= $order['id'] ?>)" data-id="<?= $order['id'] ?>" ><?= Yii::t('app', 'received_order') ?></a>
            </div>
        </div>
    </div>
<?php } ?>
