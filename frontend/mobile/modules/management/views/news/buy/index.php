		...
		<div class="list_content">
			...
			<?php if ($data) { ?>
				...
				foreach ($data as $item) {
					...
					$link_views = Url::to(['buy-view', 'id' => $item['id']]);
					$link_delete = Url::to(['buy-delete', 'id' => $item['id']]);
				?>
					<div class="item_list wow fadeInUp">
						<ul>
							<li>#<?= $i++ ?> </li>
							<li class="title">
								<span>Người bán: <?= $item['username'] ?></span>
	...
	<script type="text/javascript">
		...
		$(document).on('click', '.delete-all', function() {
			if (confirm('Bạn có chắc chắn muốn xóa tất cả mục đã chọn?')) {
				...
				loadAjaxPost('<?= Url::to(['/management/news/buy-delete-all']) ?>', items, $(this).parent());
			}
		});
