...
<style type="text/css">
    ...
    body .solid-out .solid-out-circle {
        background: #c3f0b4;
        color: #000;
        font-size: 13px;
        font-weight: bold;
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#delete-all').click(function (){
            confirmCS('<?= Yii::t('app', 'are_you_delete') ?>', {});
    ...
    jQuery(document).on('click', '.selectedbt', function () {
        ...
        $(this).html('<?= Yii::t('app', 'select') ?>');
        event.stopPropagation();
    });
    jQuery(document).on('click', '.delete-product', function () {
        $(this).addClass('box-waiting-confirm-js');
        confirmCS('<?= Yii::t('app', 'are_you_delete') ?>', {});
        yesConFirm =  function(option) {
            _this = $('.box-waiting-confirm-js').first();
            _this.removeClass('box-waiting-confirm-js');
    ...
    jQuery(document).on('click', '.do-not-product', function () {
        $(this).addClass('box-waiting-confirm-js');
        confirmCS('<?= Yii::t('app', 'are_sure_change_to_over') ?>', {});
        yesConFirm =  function(option) {
            _this = $('.box-waiting-confirm-js').first();
            _this.removeClass('box-waiting-confirm-js');
            var id = _this.attr('data');
    ...
    jQuery(document).on('click', '.do-have-product', function () {
        $(this).addClass('box-waiting-confirm-js');
        confirmCS('<?= Yii::t('app', 'are_sure_change_to_having') ?>', {});
        yesConFirm =  function(option) {
            _this = $('.box-waiting-confirm-js').first();
            _this.removeClass('box-waiting-confirm-js');
            var id = _this.attr('data');
