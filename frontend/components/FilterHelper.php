...
class FilterHelper {
    ...
    public function getAttributesFilter($att_set_id) {
        if (is_null($this->attributesFilter)) {
            $conditions = 'is_filterable = 1 AND (frontend_input=:frontend_input_select OR frontend_input=:frontend_input_multiselect OR frontend_input=:frontend_input_textnumber OR frontend_input=:frontend_input_price) AND (attribute_set_id=:attribute_set_id OR is_system=1) ORDER BY sort_order ASC LIMIT 30';
            $params = [
                ':frontend_input_select' => 'select',
                ':frontend_input_multiselect' => 'multiselect',
                ':frontend_input_textnumber' => 'textnumber',
                ':frontend_input_price' => 'price',
                ':attribute_set_id' => $att_set_id
            ];
            $attributes = ProductAttribute::find()->where($conditions, $params)->all();
            if (count($attributes)) {
    ...
    public function getAttributesSystemFilter($options = array()) {
        if (is_null($this->attributesFilter)) {
            $condition = 'is_filterable=1 AND (frontend_input=:frontend_input_select OR frontend_input=:frontend_input_multiselect OR frontend_input=:frontend_input_textnumber OR frontend_input=:frontend_input_price) AND is_system=:is_system ORDER BY sort_order ASC limit 30';
            $params = [
                ':frontend_input_select' => 'select',
                ':frontend_input_multiselect' => 'multiselect',
                ':frontend_input_textnumber' => 'textnumber',
            ...
            $attributes = ProductAttribute::find()->where($condition, $params)->all();
            if (count($attributes)) {
                foreach ($attributes as $att) {
    ...
    private function createUrlFilter($attribute, $option, $checked = false, $options = array()) {
        ...
        $curentOption = Yii::$app->request->get($key, '');
    ...
    private function createUrl($params = array(), $unsetKey = array(), $options = array()) {
        if (isset($options['route']) && $options['route']) {
            ...
            $rounte = Url::home() . Yii::$app->request->getPathInfo();
            // $rounte = Yii::$app->request->getHostInfo() . Yii::$app->request->getPathInfo();
        }
        //
        ...
        if (key_exists(ClaLid::PAGE_VAR, $paramsNew)) {
            unset($paramsNew[ClaLid::PAGE_VAR]);
        }
        ...
        if (isset($options['category']) && $options['category'] && isset($options['alias_mn']) && $options['alias_mn']) {
            ...
            $route = Url::to([
                        ...
                        'id' => $options['category']['cat_id'],
                        'alias' => $alias
            ]);
            $url = $route . ((!empty($queryString)) ? '?' . $queryString : '');
