...
class AttributeHelper {
    ...
    public static function getDynamicProduct($pro, $attributesShow) {
        $results = array();
        $dynamic = ($pro->dynamic_field) ? json_decode($pro->dynamic_field) : array();
        if (!empty($dynamic)) {
            foreach ($dynamic as $objAtt) {
                if (isset($attributesShow[$objAtt->id])) {
                    ...
                    if (isset($objAtt->value_child) && $objAtt->value_child) {
                        ...
                        $results[$objAtt->id]['value'] = self::getValueAttribute($attributesShow[$objAtt->id], $objAtt->index_key, $objAtt->value);
                        //hatv get ext to select color detal page
                        $results[$objAtt->id]['ext'] = self::getExtAttribute($attributesShow[$objAtt->id], $objAtt->index_key, $objAtt->value);
                    }
                }
            }
        }
    ...
    public static function getValueAttribute($attribute, $index_key, $value = "") {
        if ($attribute['frontend_input'] == 'select') {
            return ProductAttributeOption::getValueByKey($index_key);
        } elseif ($attribute['frontend_input'] == 'multiselect') {
            if (is_array($index_key) && !empty($index_key)) {
                $val = array();
                foreach ($index_key as $ikey) {
                    $v = ProductAttributeOption::getValueByKey($ikey, $attribute['id']);
                    if ($v)
                        $val[] = $v;
                }
    ...
    public static function getExtAttribute($attribute, $index_key, $value = "") {
        if ($attribute['frontend_input'] == 'select') {
            return ProductAttributeOption::getExtByKey($index_key);
        } elseif ($attribute['frontend_input'] == 'multiselect') {
            if (is_array($index_key) && !empty($index_key)) {
                $val = array();
                foreach ($index_key as $ikey) {
                    $v = ProductAttributeOption::getExtByKey($ikey, $attribute['id']);
                    if ($v)
                        $val[] = $v;
                }
                return empty($val) ? '' : $val;
     ...
     */
    static function getConfigurableAttributes($att_set_id = 0) {
        $atttibutes = array();
        $_attributes = ProductAttribute::find()->where('is_configurable=1 AND attribute_set_id=:attribute_set_id', [
                    ':attribute_set_id' => $att_set_id
                ])->orderBy('sort_order ASC')->limit(3)->all();
        if (count($_attributes)) {
            foreach ($_attributes as $att) {
     ...
     */
    static function geProducttConfigurableValues($product = array(), $isArray = true) {
        $configs = array();
        if (!$isArray) {
            $proConfigs = \common\models\product\ProductConfigurableValue::find()
                            ->where('product_id=:product_id', [':product_id' => $product['id']])
                            ->orderBy('order ASC')->limit(30)->all();
        } else {
            $proConfigs = (new \yii\db\Query())->select('*')
     ...
     * @param type $product
     */
    static function getConfiguableFilter($att_set_id = 0, $product = array()) {
        // các attributes dc set là configurable của
        $attributeConfigurables = self::getConfigurableAttributes($att_set_id);
        //Các giá trị configurable của product
        $productConfigurableValues = self::geProducttConfigurableValues($product);
    ...
    function getChangePriceProduct($product_id, $att_set_id = 0) {
        ...
        if ($product_id) {
            ...
            $_atttibutes = ProductAttribute::find()->where('is_change_price = 1 AND attribute_set_id=:attribute_set_id', [
                        ':attribute_set_id' => $att_set_id
                    ])->limit(10)->orderBy('sort_order ASC')->all();

            if (count($_atttibutes)) {
                $count = ProductAttributeOptionPrice::getCountByProduct($product_id);
                if ($count) {
                    foreach ($_atttibutes as $att) {
                        $options = ProductAttributeOptionPrice::getOptionProduct($product_id, $att->id);
                        if (!empty($options)) {
                            $atttibutes[$att->id] = $att->attributes;
