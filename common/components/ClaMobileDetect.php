...
class ClaMobileDetect {
    ...
    public function isMobile($userAgent = null, $httpHeaders = null) {
        ...
        if ($this->checkHttpHeadersForMobile()) {
            ...
            // set rieng cho ben vinhome
            if (Yii::$app->id == 'public') {
                if (in_array(Yii::$app->theme->name, array('w3ni380', 'w3ni390', 'w3ni447','w3ni481'))) {
                    return true;
                }
            }
            // Nếu là ipad thì hiển thị như pc
            return false;
        } else {
    ...
    public function isIPad() {
        // để bản ipad là bản web
        return false;
        $this->setDetectionType(self::DETECTION_TYPE_MOBILE);
        $ipadRegex = self::$tabletDevices['iPad'];
        if ($this->match($ipadRegex)) {
            return true;
        }
        return false;
    }
    ...
    public function isActiceApp() {
        if(isset($_SESSION['active_app_gcaeco']) && $_SESSION['active_app_gcaeco']) {
            return $_SESSION['active_app_gcaeco'];
        }
        return false;
    }
