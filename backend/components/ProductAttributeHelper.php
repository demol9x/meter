...
class ProductAttributeHelper {
     ...
     * 
     */
    public static function attRenderHtmlAll($attribute_set_id, $product = null) {
        $values = ($product) ? self::getDynamicValueProduct($product) : array();
        //
    ...
    public static function attInputTemplate($attribute, $htmlOptions = array(), $data = array()) {
        ...
        if (!empty($html)) {
            ...
            if (empty($class)) {
                ...
                $output .= '<div class="controls col-sm-10"><div class="col-sm-5">' . $html . '</div></div>';
                $output .= "</div>";
            }
        }
        return $output;
    ...
    public static function attRenderInputHtml($attribute, $htmlOptions = array(), $data = array()) {
        ...
        switch ($attribute['frontend_input']) {
            ...
            case 'textarea':
                ...
                break;
            case 'select':
                $htmlOptions = array_merge($htmlOptions, array('prompt' => '-- Hãy chọn--', 'class' => 'form-control span12 col-sm-4'));
