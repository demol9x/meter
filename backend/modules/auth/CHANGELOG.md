...
- Enh: Custom side menu via `backend\modules\auth\Module::items` (mdmunir).
...
- Chg: Can customize return of `backend\modules\auth\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `backend\modules\auth\components\BizRule` (mdmunir).
...
- Chg: Remove `backend\modules\auth\Module::allowActions`. Set access control directly with `backend\modules\auth\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `backend\modules\auth\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
