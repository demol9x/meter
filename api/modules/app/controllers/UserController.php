...
class UserController extends LoginedController
{
    ...
    function actionCheckPaymentCoin()
    {
        $post = $this->getDataPost();
        $resonse = $this->getResponse();
        if (isset($post['money']) && $post['money'] && isset($post['type']) !== false && $post['type'] !== '') {
            $gcoin = \common\models\gcacoin\Gcacoin::findOne($post['user_id']);
            $kt = false;
            $money = \common\models\gcacoin\Gcacoin::getCoinToMoney($post['money']);
            switch ($post['type']) {
                case \common\models\gcacoin\GcaCoinHistory::TYPE_V:
                    $dv = __VOUCHER;
                    $kt = $gcoin->addCoin(-$money);
                    break;
                case \common\models\gcacoin\GcaCoinHistory::TYPE_V_RED:
                    $dv = __VOUCHER_RED;
                    $kt = $gcoin->addCoinRed(-$money);
                    break;
                case \common\models\gcacoin\GcaCoinHistory::TYPE_V_SALE:
                    $dv = __VOUCHER_SALE;
                    $kt = $gcoin->addCoinSale(-$money);
                    break;
                default:
                    $resonse['message'] = "Không thể xác định loại voucher cần kiểm tra ";
                    $resonse['data'] = $post;
                    return $this->responseData($resonse);
            }
            if ($kt) {
                $resonse['data'] = $post;
                $resonse['message'] = "Tài khoản đủ điều kiện thanh toàn bằng " . $dv;
                $resonse['code'] = 1;
                return $this->responseData($resonse);
            } else {
                $resonse['error'] = "Tài khoản không đủ điều kiện thanh toàn bằng " . $dv;
                $resonse['data'] = $post;
                return $this->responseData($resonse);
            }
        }
        $resonse['error'] = "Không đủ dữ liệu để kiểm tra.";
        $resonse['data'] = $post;
        return $this->responseData($resonse);
    }

    function actionGetUserCoin()
    {
        $post = $this->getDataPost();
        $resonse = $this->getResponse();
        $gcoin = \common\models\gcacoin\Gcacoin::getModel($post['user_id']);
        $resonse['data'] = $gcoin ? $gcoin->attributes : [];
        $resonse['code'] = 1;
    ...
    public function actionGetDataShop()
    {
        $this->setTimeLoadOnce(2);
        $shop = Shop::findOne(['user_id' => $this->user->id]);
        $resonse['data'] = [];
        $resonse['code'] = 1;
        $resonse['error'] = "Người dùng không phải doanh nghiệp.";
        if (isset($shop) && $shop) {
            $resonse['data'] = $shop->attributes;
            $resonse['error'] = "Lấy thông tin tài khoản thành công.";
        }
        return $this->responseData($resonse);
    }
            ...
            if ($model->save()) {
                if ($model->time_limit_type_term == 0) {
                    ...
                    if ($coin->addCoin(-$xu) && $coin->save(false)) {
                        ...
                        $resonse['error'] = 'Quý khách không đủ V để thanh toán. Tài khoản quý khách sẽ chuyển về gói gói miễn phí 12 tháng đầu. Quý lòng Nạp V và cập nhật lại gói.';
                    }
                }
